...
|----------|-------------|
| **Claude Code** | Installs as a skill — type `/app-matrix:go` |
...
# Install the plugin
/plugin install app-matrix@run-matrix
```

Then type `/app-matrix:go` to run it.
...
Type `/app-matrix:go` and describe your app idea.
